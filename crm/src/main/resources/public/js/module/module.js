layui.use(['table', 'treetable'], function () {
    ...
    table.on('tool(munu-table)', function (obj) {
        ...
        if (layEvent === 'add') {
            ...
            layer.confirm('确定删除当前菜单？', {icon: 3, title: "菜单管理"}, function (index) {
                $.post(ctx+"/modelus/delete",{id:data.id},function (data) {
                    if(data.code==200){
                        layer.msg("操作成功！");
    ...
    function openAddModuleDialog(grade,parentId){
        var grade=grade;
        var url  =  ctx+"/modelus/toAddModulePage?grade="+grade+"&parentId="+parentId;
        var title="菜单添加";
        layui.layer.open({
            title : title,
            type : 2,
    ...
    function openUpdateModuleDialog(id){
        var url  =  ctx+"/modelus/toUpdateModulePage?id="+id;
        var title="菜单更新";
        layui.layer.open({
            title : title,
            type : 2,
