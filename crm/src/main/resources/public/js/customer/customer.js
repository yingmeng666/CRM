layui.use(['table', 'layer', "form"], function () {
    ...
    table.on('tool(customers)', function (obj) {
        ...
        if (layEvent === "edit") {
            ...
            layer.confirm("确认删除当前记录?", {icon: 3, title: "客户管理"}, function (index) {
                $.post(ctx + "/customers/delete", {id: obj.data.id}, function (data) {
                    if (data.code == 200) {
                        layer.msg("删除成功");
                        tableIns.reload();
                    } else {
    ...
    function openAddOrUpdateCustomerDialog(id) {
        ...
        var url = ctx + "/customers/toAddOrUpdateCustomerPage";
        if (id != null && id != "") {
    ...
    function openOrderInfoDialog(data) {
        ...
        layui.layer.open({
            ...
            area: ["700px", "500px"],
            maxmin: true,
            content: ctx + "/customers/toOrderInfoPage?customerId=" + data[0].id
        })
